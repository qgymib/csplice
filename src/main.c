...
static int _splice_tree_finialize_append_node(lua_State *L, int idx, pass_node_t *node)
{
    if (node->type == PASS_NODE_TYPE_VIRT)
    {
        ...
        for (i = 0; i < node->child_sz; i++)
        {
            _splice_tree_finialize_append_node(L, idx, node->childs[i]);
        }
        return 0;
    }
    ...
    if (node->type == PASS_NODE_TYPE_FILE)
    {
        ...
        lua_pushfstring(L,
                        ...
                        "////////////////////////////////////////////////////////////////////////////////\n"
                        "%s\n",
                        node, node->data.file.path.str, node->data.file.data.str);
...
static int _pmain(lua_State *L)
{
    ...
    if (_G->ofile != NULL)
    {
        _splice_tree_finialize(L, _G->root, _G->ofile);
    }
